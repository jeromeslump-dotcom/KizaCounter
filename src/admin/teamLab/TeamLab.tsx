...
import { completeZones, METRICS, theoreticalSummary, type TeamLabMode, zoneSummary } from "./teamLabData";
...
export default function TeamLab({ open, onClose, onBack }: TeamLabProps) {
  ...
  const stepLabel = mode === "combats" ? "1" : mode === "formations" ? "2" : "3";
  const description =
    mode === "combats"
  ...
  return (
    <div className="fixed inset-0 z-[75] flex items-center justify-center bg-black/75 p-3 backdrop-blur-sm sm:p-4" onClick={onClose} role="presentation">
      <section className="ui-modal flex max-h-[95vh] w-full max-w-[1500px] flex-col overflow-hidden rounded-3xl border shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="team-lab-title" onClick={(event) => event.stopPropagation()}>
        <header className="ui-modal-header shrink-0 p-5 sm:p-6">
          <div className="ui-modal-header-inner">
            <div>
              <h2 id="team-lab-title" className="ui-text-primary text-xl font-black">
                🧪 Team Lab — Étape {stepLabel}
              </h2>
              <p className="ui-text-secondary mt-1 text-xs sm:text-sm">{description}</p>
            </div>
            <button type="button" onClick={onClose} className="ui-button-icon" aria-label="Fermer">✕</button>
          </div>
        </header>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-6">
          {mode === "theoretical" ? (
            <>
              <div className="mb-5 flex flex-wrap items-center gap-4 text-xs">
                <span className="flex items-center gap-2">
                  <span className="h-3 w-3 rounded-full" style={{ background: "var(--ui-theme-primary)" }} aria-hidden="true" />
            ...
            <>
              <div className="mb-5 flex flex-wrap items-center gap-4 text-xs">
                <span className="flex items-center gap-2"><span className="h-3 w-3 rounded-sm" style={{ background: "var(--ui-success)" }} aria-hidden="true" />WIN</span>
                <span className="flex items-center gap-2"><span className="h-3 w-3 rounded-sm" style={{ background: "var(--ui-theme-secondary)" }} aria-hidden="true" />LOSS</span>
                <span className="ui-text-muted">WIN / LOSS = {mode === "combats" ? "combats enregistrés" : "formations enregistrées"}</span>
