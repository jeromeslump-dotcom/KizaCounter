...
export default function AdminPanel({
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/75 p-3 backdrop-blur-sm sm:p-4" onClick={onClose} role="presentation">
      <section className="ui-modal w-full max-w-lg overflow-hidden rounded-3xl border shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="admin-panel-title" onClick={(event) => event.stopPropagation()}>
        <header className="border-b ui-divider p-5 sm:p-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h2 id="admin-panel-title" className="ui-text-primary text-xl font-black">⚙️ Admin Panel</h2>
              <p className="ui-text-secondary mt-1 text-xs sm:text-sm">Gestion de l'application</p>
            </div>
