...
import AnalysisHelpResults, { type HeroEvaluationGroup } from "./AnalysisHelpResults";
...
export default function AnalysisHelp({ open, heroes, combats, onClose, onBack }: AnalysisHelpProps) {
  ...
  const matchingCombats = useMemo(
    () => enemyIds.length === TEAM_SIZE ? combats.filter((combat) => sameTeam(enemyIds, combat.enemy_heroes ?? [])) : [],
    [combats, enemyIds]
  );
  ...
  const combatGroups = useMemo<HeroEvaluationGroup[]>(() => {
    ...
    for (const combat of matchingCombats) {
      ...
      const evaluation = team.length === TEAM_SIZE ? evaluateTeam(team, combats, usage, enemyIds) : null;
      const scoreA = evaluation ? evaluation.historicalWinRate * settings.teamA.generalWinRateWeight : null;
      const scoreB = evaluation ? evaluation.historicalWinRate * settings.teamB.generalWinRateWeight : null;
    ...
    <div className="fixed inset-0 z-[75] flex items-center justify-center bg-black/75 p-2 backdrop-blur-sm sm:p-4">
      <section className="ui-modal flex max-h-[96vh] w-full max-w-7xl flex-col overflow-hidden rounded-3xl border shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="analysis-help-title">
        <header className="flex items-start justify-between gap-4 border-b ui-divider p-5 sm:p-6">
          <div>
            ...
            <p className="ui-text-secondary mt-1 text-xs sm:text-sm">
              {showResults ? "Résultats des combats correspondant exactement à l'équipe ennemie." : "Sélectionnez exactement les 5 ennemis à analyser."}
            </p>
          </div>
        ...
        <footer className="flex justify-between gap-2 border-t ui-divider px-4 py-3 sm:px-5 sm:py-4">
          <button type="button" onClick={showResults ? () => setShowResults(false) : onBack} className="ui-action rounded-lg border px-4 py-2 text-xs font-bold transition">
