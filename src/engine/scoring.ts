...
function calculateCore4ModulePoints(teamIds: string[], enemyIds: string[], combats: Parameters<typeof analyzeCore4Plus1>[1], settings: ReturnType<typeof getEngineSettings>, maxPoints: number): number {
  ...
  for (const analysis of analyses) {
    ...
    const confidence = historicalConfidence(analysis.battles, settings.advanced.core4ConfidenceBattles);
    bestRawScore = Math.max(bestRawScore, (analysis.winRate / 100) * confidence);
  }
  return normalizeModulePoints(bestRawScore, maxPoints);
}

export function evaluateTeam(team: Hero[], combats: Parameters<typeof evaluateTeamHistory>[1], _usage: Record<string, HeroUsage> | undefined, enemyIds: string[]): TeamEvaluation {
...
export function scoreTeam(team: Hero[], combats: Parameters<typeof evaluateTeamHistory>[1], usage: Record<string, HeroUsage> | undefined, enemyIds: string[]): TeamScore {
  return { heroIds: team.map((hero) => hero.id), score: evaluateTeam(team, combats, usage, enemyIds).score };
}

export function recommendTeam(enemyIds: string[], heroes: Hero[], combats: Parameters<typeof evaluateTeamHistory>[1], onSource?: RecommendationSourceCallback): Hero[] {
  if (!enemyIds.length) { onSource?.("fallback"); return []; }
  const settings = getEngineSettings();
  const historicalTeam = findBestHistoricalTeam(enemyIds, combats, heroes);
  if (historicalTeam && historicalTeam.length === TEAM_SIZE) { onSource?.("exact-history"); return historicalTeam; }
  const availableHeroes = heroes;
  if (availableHeroes.length <= TEAM_SIZE) { onSource?.("fallback"); return availableHeroes; }
  const heroesById = new Map(heroes.map((hero) => [hero.id, hero]));
  const counterUsage = calculateCounterUsage(enemyIds, combats);
  const ranked = availableHeroes.map((hero) => ({ hero, score: counterHeroScore(hero, counterUsage, settings) })).sort((a, b) => b.score - a.score || a.hero.name.localeCompare(b.hero.name));
  ...
  if (core4Analyses.length > 0) {
    ...
    for (const analysis of core4Analyses) {
      const core4Heroes = analysis.coreIds.map((id) => heroesById.get(id)).filter((hero): hero is Hero => Boolean(hero));
      if (core4Heroes.length !== TEAM_SIZE - 1) continue;
      const coreConfidence = historicalConfidence(analysis.battles, settings.advanced.core4ConfidenceBattles);
      const coreScore = analysis.winRate * coreConfidence;
      const core4Ids = new Set(core4Heroes.map((hero) => hero.id));
      const replacementsByHeroId = new Map(analysis.replacements.map((entry) => [entry.heroId, entry]));
    ...
    if (bestCompleteTeam && bestCompleteTeam.length === TEAM_SIZE) { onSource?.("core4"); return bestCompleteTeam; }
  }
  const historicalClassTeam = findBestHistoricalClassTeam(enemyIds, combats, heroes);
  if (historicalClassTeam && historicalClassTeam.length === TEAM_SIZE) { onSource?.("class-history"); return historicalClassTeam; }
...
export function recommendAlternativeTeam(enemyIds: string[], heroes: Hero[], combats: Parameters<typeof evaluateTeamHistory>[1], primaryTeam: Hero[] = []): Hero[] {
  if (!enemyIds.length) return [];
  const primaryTeamKey = primaryTeam.length === TEAM_SIZE ? teamKey(primaryTeam.map((hero) => hero.id)) : "";
  if (heroes.length < TEAM_SIZE) return heroes;
  const settings = getEngineSettings();
  const budgets = getPointBudgets(settings, "B");
  const counterUsage = calculateCounterUsage(enemyIds, combats);
  const ranked = heroes.map((hero) => {
    const counter = counterUsage[hero.id];
    if (!counter) return { hero, score: 0 };
    const confidence = historicalConfidence(counter.total, settings.advanced.historicalConfidenceBattles);
  ...
  for (const combat of combats) {
    ...
    if (enemyKey === targetEnemyKey) {
      ...
      if (combat.won && myKey !== primaryTeamKey) {
        const team = myIds.map((id) => heroesById.get(id)).filter((hero): hero is Hero => Boolean(hero));
    ...
    if (targetEnemyClassKey) {
      ...
      if (classKey === targetEnemyClassKey) addHistoryStats(classHistory, `${classKey}::${myKey}`, combat.won);
    }
  }
  const isPrimaryTeam = (team: Hero[]): boolean => primaryTeamKey !== "" && teamKey(team.map((hero) => hero.id)) === primaryTeamKey;
  const addCandidate = (candidates: Map<string, Hero[]>, team: Hero[]): void => {
    ...
    const normalizedTeam = ids.map((id) => heroesById.get(id)).filter((hero): hero is Hero => Boolean(hero));
  ...
  const evaluateAlternative = (team: Hero[]) => {
    ...
    const generalWinRatePoints = historyBattles > 0 ? normalizeModulePoints(historyWinRate / 100, budgets.generalWinRate) : 0;
    ...
    const classStats = targetEnemyClassKey ? classHistory.get(`${targetEnemyClassKey}::${myKey}`) ?? { wins: 0, losses: 0 } : { wins: 0, losses: 0 };
    const classBattles = classStats.wins + classStats.losses;
    const classWinRate = calculateWinRate(classStats.wins, classBattles);
    const classHistoryConfidenceBattles = Math.max(1, settings.advanced.historicalConfidenceBattles * 2);
    const classHistoryConfidence = historicalConfidence(classBattles, classHistoryConfidenceBattles);
    const classHistoryPoints = classBattles > 0 ? normalizeModulePoints((classWinRate / 100) * classHistoryConfidence, budgets.generalWinRate) : 0;
    const losingHistory = historyBattles > 0 && history.wins === 0;
    const fallbackPoints = exact.wins + exact.losses > 0 ? specificHistoryPoints : classBattles > 0 ? classHistoryPoints : generalWinRatePoints;
    return { team, score: specificHistoryPoints * settings.teamB.specificHistoryWeight + core4Points * settings.teamB.core4Weight + fallbackPoints * settings.teamB.generalWinRateWeight - (losingHistory ? Number.MAX_SAFE_INTEGER : 0), history: { wins: history.wins, losses: history.losses, battles: historyBattles, winRate: historyWinRate } };
  };
