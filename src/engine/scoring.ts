...
function isSameEnemyTeam(enemyIds: string[], combatEnemyIds: string[]): boolean {
...
export function calculateSpecificHistoryPoints(
  ...
  const confidenceBattles = getEngineSettings().advanced.teamAHistoricalConfidenceBattles;
...
export function coverageReport(enemyIds: string[], teamIds: string[], combats: Combat[]): CoverageReport {
  ...
  const heroes = team.map((heroId) => {
    ...
    const confidence = Math.min(battles / Math.max(1, settings.advanced.core4ConfidenceBattles), 1);
...
export function scoreHero(hero: Hero): HeroScore { return { heroId: hero.id, score: 0 }; }
...
export function evaluateTeamHistory(teamIds: string[], combats: Combat[]): { wins: number; losses: number; battles: number; winRate: number } {
  const team = uniqueIds(teamIds);
  if (team.length === 0) return { wins: 0, losses: 0, battles: 0, winRate: 0 };
  let wins = 0;
  let losses = 0;
  for (const combat of combats) {
...
function calculateCore4ModulePoints(teamIds: string[], enemyIds: string[], combats: Combat[], settings: ReturnType<typeof getEngineSettings>, maxPoints: number): number {
  ...
  for (const analysis of analyses) {
    ...
    const confidence = Math.min(analysis.battles / Math.max(1, settings.advanced.core4ConfidenceBattles), 1);
    bestRawScore = Math.max(bestRawScore, (analysis.winRate / 100) * confidence);
  }
  return normalizeModulePoints(bestRawScore, maxPoints);
}

export function evaluateTeam(team: Hero[], combats: Combat[], _usage: Record<string, HeroUsage> | undefined, enemyIds: string[]): TeamEvaluation {
  const settings = getEngineSettings();
  ...
  const generalWinRatePoints = normalizeModulePoints(history.battles > 0 ? history.winRate / 100 : 0, budgets.generalWinRate);
  const exactHistory = enemyIds.length === TEAM_SIZE ? evaluateExactTeamHistory(teamIds, enemyIds, combats) : { wins: 0, losses: 0, battles: 0, winRate: 0 };
  const specificHistoryPoints = calculateSpecificHistoryPoints(exactHistory.wins, exactHistory.losses, budgets.specificHistory);
  const core4Points = calculateCore4ModulePoints(teamIds, enemyIds, combats, settings, budgets.core4);
  const score = specificHistoryPoints * settings.teamA.specificHistoryWeight + core4Points * settings.teamA.core4Weight + generalWinRatePoints * settings.teamA.generalWinRateWeight;
  return { score, historicalWins: history.wins, historicalLosses: history.losses, historicalBattles: history.battles, historicalWinRate: history.winRate };
}

export function scoreTeam(team: Hero[], combats: Combat[], usage: Record<string, HeroUsage> | undefined, enemyIds: string[]): TeamScore {
  return { heroIds: team.map((hero) => hero.id), score: evaluateTeam(team, combats, usage, enemyIds).score };
}
...
export function findBestHistoricalTeam(enemyIds: string[], combats: Combat[], heroes: Hero[]): Hero[] | null {
  ...
  winningCandidates.sort((a, b) => {
    ...
    return bReliability(a.wins, aBattles) - bReliability(a.wins, aBattles) || bBattles - aBattles || b.wins - a.wins || teamKey(a.heroIds).localeCompare(teamKey(b.heroIds));
  });
...
export function evaluateEnemyClassHistory(teamIds: string[], enemyIds: string[], combats: Combat[], heroes: Hero[]): { wins: number; losses: number; battles: number; winRate: number; classKey: string | null } {
  const team = uniqueIds(teamIds);
  ...
  for (const combat of combats) {
    ...
    if (historicalEnemy.length !== TEAM_SIZE || getEnemyClassKey(historicalEnemy, heroes) !== targetClassKey) continue;
    combat.won ? wins++ : losses++;
  }
  const battles = wins + losses;
...
function findBestHistoricalClassTeam(enemyIds: string[], combats: Combat[], heroes: Hero[]): Hero[] | null {
  ...
  const ordered = [...candidates.values()].filter((candidate) => candidate.wins > 0).sort((a, b) => {
    const aBattles = a.wins + a.losses;
    const bBattles = b.wins + b.losses;
    const aReliability = aBattles > 0 ? (a.wins / aBattles) * (settings.advanced.teamAHistoricalReliabilityBase + settings.advanced.teamAHistoricalReliabilityConfidenceWeight * (aBattles / (aBattles + confidenceBattles))) : 0;
    const bReliability = bBattles > 0 ? (b.wins / bBattles) * (settings.advanced.teamAHistoricalReliabilityBase + settings.advanced.teamAHistoricalReliabilityConfidenceWeight * (bBattles / (bBattles + confidenceBattles))) : 0;
...
function calculateCounterUsage(enemyIds: string[], combats: Combat[]): Record<string, { wins: number; losses: number; total: number; winRate: number }> {
  ...
  for (const entry of Object.values(result)) entry.winRate = entry.total > 0 ? (entry.wins / entry.total) * 100 : 0;
  return result;
}

function counterHeroScore(hero: Hero, counterUsage: Record<string, { wins: number; losses: number; total: number; winRate: number }>): number {
  const settings = getEngineSettings();
  const counter = counterUsage[hero.id];
  if (!counter || counter.total <= 0) return 0;
  const confidence = Math.min(counter.total / Math.max(1, settings.advanced.teamAHistoricalConfidenceBattles), 1);
  return counter.winRate * confidence * settings.advanced.teamACounterWinRateMultiplier * settings.teamA.specificHistoryWeight;
}

export function recommendTeam(enemyIds: string[], heroes: Hero[], combats: Combat[], onSource?: RecommendationSourceCallback): Hero[] {
  if (!enemyIds.length) {
    onSource?.("fallback");
    return [];
  }
  ...
  const ranked = availableHeroes.map((hero) => ({ hero, score: counterHeroScore(hero, counterUsage) })).sort((a, b) => b.score - a.score || a.hero.name.localeCompare(b.hero.name));
  ...
  if (core4Analyses.length > 0) {
    ...
    for (const analysis of core4Analyses) {
      const core4Heroes = analysis.coreIds.map((id) => heroes.find((hero) => hero.id === id)).filter((hero): hero is Hero => Boolean(hero));
      if (core4Heroes.length !== TEAM_SIZE - 1) continue;
      const coreConfidence = Math.min(analysis.battles / Math.max(1, settings.advanced.core4ConfidenceBattles), 1);
      ...
      for (const candidate of ranked) {
        ...
        const replacement = analysis.replacements.find((entry) => entry.heroId === candidate.hero.id);
        if (!replacement) continue;
        const replacementScore = replacement.score;
        const core4Points = normalizeModulePoints(coreScore / 100, budgets.core4);
        const completeScore = core4Points * settings.teamA.core4Weight + replacementScore * settings.teamA.core4Weight * (budgets.core4 / 100);
        const completeTeam = [...core4Heroes, candidate.hero];
        const completeTeamKey = teamKey(completeTeam.map((hero) => hero.id));
        const bestCompleteTeamKey = bestCompleteTeam ? teamKey(bestCompleteTeam.map((hero) => hero.id)) : "";
        if (completeScore > bestCompleteScore || (completeScore === bestCompleteScore && (coreScore > bestCoreScore || (coreScore === bestCoreScore && (replacementScore > bestReplacementScore || (replacementScore === bestReplacementScore && completeTeamKey.localeCompare(bestCompleteTeamKey) < 0)))))) {
...
export function recommendAlternativeTeam(enemyIds: string[], heroes: Hero[], combats: Combat[], primaryTeam: Hero[] = []): Hero[] {
  ...
  const counterUsage = calculateCounterUsage(enemyIds, combats);
  const ranked = availableHeroes.map((hero) => {
    const counter = counterUsage[hero.id];
    if (!counter) return { hero, score: 0 };
    const confidence = Math.min(counter.total / Math.max(1, settings.advanced.teamAHistoricalConfidenceBattles), 1);
    return { hero, score: counter.winRate * confidence * settings.advanced.teamBCounterWinRateMultiplier * settings.teamB.specificHistoryWeight };
  ...
  const getCore4Points = (team: Hero[]): number => {
    ...
    for (const core of core4Analyses) {
      ...
      const confidence = Math.min(core.battles / Math.max(1, settings.advanced.core4ConfidenceBattles), 1);
  ...
  const evaluateAlternative = (team: Hero[]) => {
    const teamIds = team.map((hero) => hero.id);
    const history = evaluateTeamHistory(teamIds, combats);
    const generalWinRatePoints = history.battles > 0 ? normalizeModulePoints(history.winRate / 100, budgets.generalWinRate) : 0;
    const exact = evaluateExactTeamHistory(teamIds, enemyIds, combats);
    const specificHistoryPoints = calculateSpecificHistoryPoints(exact.wins, exact.losses, budgets.specificHistory);
    const core4Points = getCore4Points(team);
    const classHistory = evaluateEnemyClassHistory(teamIds, enemyIds, combats, heroes);
    const classHistoryPoints = classHistory.battles > 0 ? normalizeModulePoints((classHistory.winRate / 100) * Math.min(classHistory.battles / Math.max(1, settings.advanced.teamAHistoricalConfidenceBattles * 2), 1), budgets.generalWinRate) : 0;
    const losingHistory = history.battles > 0 && history.wins === 0;
    const fallbackPoints = exact.battles > 0 ? specificHistoryPoints : classHistory.battles > 0 ? classHistoryPoints : generalWinRatePoints;
    return { team, score: specificHistoryPoints * settings.teamB.specificHistoryWeight + core4Points * settings.teamB.core4Weight + fallbackPoints * settings.teamB.generalWinRateWeight - (losingHistory ? Number.MAX_SAFE_INTEGER : 0), history };
  };
  const candidates: Hero[][] = [baseAlternative];
  const pool = ranked.filter((candidate) => !primaryIds.has(candidate.hero.id)).map((candidate) => candidate.hero);
  for (let index = 0; index < baseAlternative.length; index++) {
    for (const replacement of pool) {
      if (baseAlternative.some((hero, heroIndex) => heroIndex !== index && hero.id === replacement.id)) continue;
  ...
  for (const combat of combats) {
    if (!isSameEnemyTeam(enemyIds, combat.enemy_heroes ?? []) || !combat.won) continue;
    const ids = uniqueIds(combat.my_heroes ?? []);
    if (ids.length !== TEAM_SIZE || ids.some((id) => primaryIds.has(id))) continue;
    const team = ids.map((id) => heroes.find((hero) => hero.id === id)).filter((hero): hero is Hero => Boolean(hero));
  ...
  for (const candidate of candidates.slice(1)) {
    ...
    if (evaluation.score > best.score || (evaluation.score === best.score && evaluation.history.winRate > best.history.winRate) || (evaluation.score === best.score && evaluation.history.winRate === best.history.winRate && evaluation.history.wins > best.history.wins) || (evaluation.score === best.score && evaluation.history.winRate === best.history.winRate && evaluation.history.wins === best.history.wins && evaluation.history.battles > best.history.battles)) best = evaluation;
  }
  return best.team;
}
...
export function rankHeroes(heroes: Hero[], combats: Combat[]): HeroScore[] {
  void combats;
  return heroes.map((hero) => scoreHero(hero)).sort((a, b) => b.score - a.score || a.heroId.localeCompare(b.heroId));
}
