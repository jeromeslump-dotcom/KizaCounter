...
export function findBestHistoricalTeam(
  ...
  winningCandidates.sort((a, b) => {
    ...
    return (
      calculateHistoricalReliability(b.wins, aBattles) - calculateHistoricalReliability(a.wins, aBattles) ||
      bBattles - aBattles ||
      b.wins - a.wins ||
...
function getEnemyClassKey(enemyIds: string[], heroes: Hero[]): string | null {
  const classes = enemyIds
    ...
    .filter((cls): cls is Hero["cls"] => cls === "STR" || cls === "AGI" || cls === "INT");
...
export function evaluateEnemyClassHistory(
  ...
  combats: Combat[],
  heroes: Hero[]
): { wins: number; losses: number; battles: number; winRate: number; classKey: string | null } {
  const team = uniqueIds(teamIds);
...
export function recommendAlternativeTeam(enemyIds: string[], heroes: Hero[], combats: Combat[], primaryTeam: Hero[] = []): Hero[] {
  ...
  const evaluateAlternative = (team: Hero[]) => {
    ...
    return {
      team,
      score: specificHistoryPoints * settings.teamB.specificHistoryWeight + core4Points * settings.teamB.core4Weight + fallbackPoints * settings.teamB.generalWinRateWeight - (losingHistory ? Number.MAX_SAFE_INTEGER : 0),
      history,
    };
  };
