...
export function recommendAlternativeTeam(
  ...
  const isPrimaryTeam = (team: Hero[]): boolean =>
    primaryTeamKey !== "" &&
    teamKey(team.map((hero) => hero.id)) === primaryTeamKey;
  const addCandidate = (
    ...
    const normalizedTeam = ids
      .map((id) => heroesById.get(id))
      .filter((hero): hero is Hero => Boolean(hero));
    if (normalizedTeam.length !== TEAM_SIZE || isPrimaryTeam(normalizedTeam))
      return;
  ...
  for (let index = 0; index < TEAM_SIZE; index++) {
    const baseTeam =
      baseAlternative.length === TEAM_SIZE ? baseAlternative : [];
  ...
  evaluations.sort((a, b) => {
    ...
    if (a.history.wins !== b.history.wins)
      return b.history.wins - a.history.wins;
