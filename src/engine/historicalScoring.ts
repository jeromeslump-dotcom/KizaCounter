...
export function buildHistoricalEnemyContext(enemyIds: string[], combats: Combat[]): HistoricalEnemyContext {
  const enemyKey = teamKey(enemyIds);
  const matchingCombats: Combat[] = [];
  for (const combat of combats) {
    if (teamKey(combat.enemy_heroes ?? []) === enemyKey) matchingCombats.push(combat);
  }
...
export function calculateHistoricalReliability(wins: number, losses: number, confidenceBattles: number, base: number, confidenceWeight: number): number {
...
export function calculateSpecificHistoryPoints(wins: number, losses: number, maxPoints: number): number {
  ...
  const confidenceBattles = Math.max(1, getEngineSettings().advanced.historicalConfidenceBattles);
  const confidence = historicalConfidence(battles, confidenceBattles);
...
export function evaluateTeamHistory(teamIds: string[], combats: Combat[]) {
  ...
  let wins = 0, losses = 0;
  for (const combat of combats) { const historicalTeam = combat.my_heroes ?? []; if (uniqueIds(historicalTeam).length !== TEAM_SIZE) continue; if (teamKey(historicalTeam) !== teamKeyValue) continue; combat.won ? wins++ : losses++; }
  const battles = wins + losses;
  return { wins, losses, battles, winRate: calculateWinRate(wins, battles) };
}

export function findBestHistoricalTeam(enemyIds: string[], combats: Combat[], heroes: Hero[], context?: HistoricalEnemyContext): Hero[] | null {
  const settings = getEngineSettings();
  ...
  const candidates = new Map<string, { heroIds: string[]; wins: number; losses: number; key: string }>();
  for (const combat of historicalCombats) {
    ...
    const candidate = candidates.get(key) ?? { heroIds, wins: 0, losses: 0, key };
    combat.won ? candidate.wins++ : candidate.losses++;
    candidates.set(key, candidate);
  }
  const winningCandidates = [...candidates.values()].filter((candidate) => candidate.wins > 0 && candidate.wins >= candidate.losses);
  if (!winningCandidates.length) return null;
  const confidenceBattles = Math.max(1, settings.advanced.historicalConfidenceBattles);
  winningCandidates.sort((a,b) => {
    const aBattles=a.wins+a.losses, bBattles=b.wins+b.losses;
    const aReliability=calculateHistoricalReliability(a.wins,a.losses,confidenceBattles,settings.advanced.historicalReliabilityBase,settings.advanced.historicalReliabilityConfidenceWeight);
    const bReliability=calculateHistoricalReliability(b.wins,b.losses,confidenceBattles,settings.advanced.historicalReliabilityBase,settings.advanced.historicalReliabilityConfidenceWeight);
    return bReliability-aReliability || bBattles-aBattles || b.wins-a.wins || a.key.localeCompare(b.key);
  });
  for (const candidate of winningCandidates) {
    const team=candidate.heroIds.map((id)=>heroesById.get(id)).filter((hero): hero is Hero => Boolean(hero));
...
export function evaluateEnemyClassHistory(teamIds: string[], enemyIds: string[], combats: Combat[], heroes: Hero[]) {
  const team=uniqueIds(teamIds); const heroesById=new Map(heroes.map((hero)=>[hero.id,hero])); const targetClassKey=getEnemyClassKey(enemyIds,heroesById);
  if(team.length!==TEAM_SIZE || !targetClassKey) return {wins:0,losses:0,battles:0,winRate:0,classKey:targetClassKey};
  const teamKeyValue=teamKey(team); const classKeyCache=new Map<string,string|null>(); let wins=0,losses=0;
  for(const combat of combats){ if(teamKey(combat.my_heroes??[])!==teamKeyValue) continue; const historicalEnemy=uniqueIds(combat.enemy_heroes??[]); if(historicalEnemy.length!==TEAM_SIZE) continue; const historicalEnemyKey=teamKey(historicalEnemy); let historicalClassKey=classKeyCache.get(historicalEnemyKey); if(historicalClassKey===undefined){historicalClassKey=getEnemyClassKey(historicalEnemy,heroesById); classKeyCache.set(historicalEnemyKey,historicalClassKey);} if(historicalClassKey!==targetClassKey) continue; combat.won?wins++:losses++; }
  const battles=wins+losses; return {wins,losses,battles,winRate:calculateWinRate(wins,battles),classKey:targetClassKey};
}

export function findBestHistoricalClassTeam(enemyIds: string[], combats: Combat[], heroes: Hero[]): Hero[] | null {
  const heroesById=new Map(heroes.map((hero)=>[hero.id,hero])); const targetClassKey=getEnemyClassKey(enemyIds,heroesById); if(!targetClassKey) return null;
  const classKeyCache=new Map<string,string|null>(); const candidates=new Map<string,{heroIds:string[];wins:number;losses:number;key:string}>();
  for(const combat of combats){ const historicalEnemy=uniqueIds(combat.enemy_heroes??[]); if(historicalEnemy.length!==TEAM_SIZE) continue; const historicalEnemyKey=teamKey(historicalEnemy); let historicalClassKey=classKeyCache.get(historicalEnemyKey); if(historicalClassKey===undefined){historicalClassKey=getEnemyClassKey(historicalEnemy,heroesById); classKeyCache.set(historicalEnemyKey,historicalClassKey);} if(historicalClassKey!==targetClassKey) continue; const heroIds=uniqueIds(combat.my_heroes??[]); if(heroIds.length!==TEAM_SIZE) continue; const key=teamKey(heroIds); const candidate=candidates.get(key)??{heroIds,wins:0,losses:0,key}; combat.won?candidate.wins++:candidate.losses++; candidates.set(key,candidate); }
  const settings=getEngineSettings(); const confidenceBattles=Math.max(1,settings.advanced.historicalConfidenceBattles);
  const ordered=[...candidates.values()].filter((candidate)=>candidate.wins>0&&candidate.wins>=candidate.losses).sort((a,b)=>{const aBattles=a.wins+a.losses,bBattles=b.wins+b.losses;const aReliability=calculateHistoricalReliability(a.wins,a.losses,confidenceBattles,settings.advanced.historicalReliabilityBase,settings.advanced.historicalReliabilityConfidenceWeight),bReliability=calculateHistoricalReliability(b.wins,b.losses,confidenceBattles,settings.advanced.historicalReliabilityBase,settings.advanced.historicalReliabilityConfidenceWeight);return bReliability-aReliability||bBattles-aBattles||b.wins-a.wins||a.key.localeCompare(b.key);});
  for(const candidate of ordered){const team=candidate.heroIds.map((id)=>heroesById.get(id)).filter((hero):hero is Hero=>Boolean(hero));if(team.length===TEAM_SIZE)return team;} return null;
}

export function evaluateSpecificHistoryModule(teamIds:string[],enemyIds:string[],combats:Combat[],team:"A"|"B"){
  const history=evaluateExactTeamHistory(teamIds,enemyIds,combats); const settings=getEngineSettings(); const budgets=getPointBudgets(settings,team); const points=calculateSpecificHistoryPoints(history.wins,history.losses,budgets.specificHistory);
