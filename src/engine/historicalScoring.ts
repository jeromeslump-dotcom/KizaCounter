...
export function findBestHistoricalTeam(
  ...
  const calculateHistoricalReliability = (wins: number, battles: number) => {
    if (battles <= 0) return 0;
  ...
  winningCandidates.sort((a, b) => {
    ...
    return (
      calculateHistoricalReliability(b.wins, aBattles) -
    ...
    .sort((a, b) => {
      ...
      const aReliability =
        aBattles > 0
          ? (a.wins / aBattles) *
            (settings.advanced.teamAHistoricalReliabilityBase +
              settings.advanced.teamAHistoricalReliabilityConfidenceWeight * aConfidence)
          : 0;
      const bReliability =
        bBattles > 0
          ? (b.wins / bBattles) *
            (settings.advanced.teamAHistoricalReliabilityBase +
              settings.advanced.teamAHistoricalReliabilityConfidenceWeight * bConfidence)
          : 0;
      return (
